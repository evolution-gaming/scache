 ...
 * https://github.com/lampepfl/dotty/issues/18099. Scala's 2 implementation crashes Scalas's 3
 * compiler.
 */
private[scache] object CacheOpsCompat {
  ...
  extension[F[_], K, V](self: Cache[F, K, V]) {
     ...
     * this one allows the loading function to fail finding the value, i.e. return [[scala.None]].
     *
     ...
     * other cases it is recommended to use
     * [[com.evolution.scache.Cache.CacheOps#getOrUpdateResourceOpt]] instead as more human-readable
     * alternative.
     *
     * @param key
     ...
     *   the method may return [[scala.None]] in case `value` completes to [[scala.None]].
     */
     ...
     * [[com.evolution.scache.Cache.CacheOps#getOrUpdateResource]] is that this one allows the
