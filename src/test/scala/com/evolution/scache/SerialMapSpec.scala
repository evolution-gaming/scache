...
class SerialMapSpec extends AsyncFunSuite with Matchers {
  ...
  private def getOrUpdate[F[_]: Async] = {
    ...
    for {
      ...
      value0 <- serialMap
        .getOrUpdate(key, started.complete(()) *> deferred.get)
        .startEnsure
      _ <- started.get
  ...
  private def remove[F[_]: Concurrent] = {
    val key = "key"
    val cache = LoadingCache.of(
      LoadingCache.EntryRefs
        .empty[F, String, SerialRef[F, SerialMap.State[Int]]]
    )
    cache.use { cache =>
  ...
  private def `not leak on failures`[F[_]: Concurrent] = {
    ...
    cache.use { cache =>
      val serialMap = SerialMap(cache)
      val modifyError = serialMap
        .modify(key) { _ => TestError.raiseError[F, (Option[Int], Unit)] }
        .attempt
      for {
        value0 <- modifyError
        value1 <- cache.get(key)
