...
class ExpiringCacheSpec extends AsyncFunSuite with Matchers {
  ...
  private def expireRecords[F[_]: Async] = {
    ...
    ExpiringCache
      .of[F, Int, Int](ExpiringCache.Config(expireAfterRead = 100.millis))
      .use { cache =>
        for {
  ...
  private def notExpireUsedRecords[F[_]: Async] = {
    ExpiringCache.of[F, Int, Int](ExpiringCache.Config(50.millis)).use {
      cache =>
        val touch = for {
  ...
  private def refreshDoesNotTouch[F[_]: Async] = {
    ...
    val config =
      ExpiringCache.Config(expireAfterRead = 100.millis, refresh = refresh.some)
    ...
    ExpiringCache.of[F, Int, Int](config).use { cache =>
      ...
      for {
        ...
        release <- Deferred[F, Unit]
        value <- cache
          .put(0, 1, released.set(true) *> release.complete(()).void)
        value <- value
        _ <- Sync[F].delay { value shouldEqual none }
        value <- cache.get(0)
        _ <- Sync[F].delay { value shouldEqual 1.some }
  ...
  private def refreshFails[F[_]: Async] = {
    ...
    def valueOf(ref: Ref[F, Int]) = { (_: Int) =>
      {
        for {
  ...
  def `refresh removes entry`[F[_]: Async] = {
    val refresh = ExpiringCache.Refresh[Int](100.millis) { _ =>
      none[Int].pure[F]
    }
    ...
    ExpiringCache.of[F, Int, Int](config).use { cache =>
      ...
      for {
        ...
        release <- Deferred[F, Unit]
        value <- cache
          .put(0, 1, released.complete(true) *> release.complete(()).void)
        value <- value
        _ <- Sync[F].delay { value shouldEqual none }
        value <- cache.get(0)
        _ <- Sync[F].delay { value shouldEqual 1.some }
  ...
  object Retry {
    ...
    def apply[F[_]: Temporal, A](delay: FiniteDuration, times: Int)(
        fa: F[Option[A]]
    ): F[Option[A]] = {
