...
class CacheSpec extends AsyncFunSuite with Matchers {
  ...
  for {
    ...
    check(s"getOrElse1: $name") { (cache, metrics) =>
      for {
...
//            _         <- IO { value shouldEqual Outcome.canceled }
    ...
    check(s"put while getOrUpdate never: $name") { (cache, metrics) =>
      for {
        fiber <- cache.getOrUpdateEnsure(0) { IO.never: IO[Int] }
        value <- cache.put(0, 0)
        value <- value
        _     <- IO { value shouldEqual none }
        value <- fiber.join
    ...
    check(s"put while getOrUpdate1 never: $name") { (cache, metrics) =>
      for {
        fiber <- cache.getOrUpdate1Ensure(0) { IO.never: IO[(Int, Option[IO[Unit]])] }
        value <- cache.put(0, 0)
        value <- value
        _     <- IO { value shouldEqual none }
        value <- fiber.join
...
//            _             <- IO { cancelOutcome shouldEqual Outcome.canceled }
    ...
    check(s"each release performed exactly once during " +
      ...
      for {
        ...
        _ <- f1.join.void
        _ <- f2.join.flatMap(_.sequence)
        _ <- f3.join.flatMap(_.flatMap(_._2).sequence_)
        _ <- f4.join.flatMap(_.sequence)
        _ <- cache.clear.flatten
    ...
    check(s"failing loads don't interfere with releases during " +
      ...
      for {
        ...
        _ <- f1.join.void
        _ <- f2.join.void
        _ <- f3.join.flatMap(_.sequence)
        _ <- f4.join.flatMap(_.flatMap(_._2).sequence_)
        _ <- f5.join.flatMap(_.sequence)
        _ <- cache.clear.flatten
    ...
    check(s"modify guarantees updated value write concurrently accessing multiple keys: $name") {
      (cache, metrics) =>
        ...
        for {
          ...
          results <- List(f0, f1, f2, f3).flatTraverse(_.join)
