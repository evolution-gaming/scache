...
object Cache {
  ...
  /** Creates a cache implementation, which is able to load the missing values.
    ...
    *     there is no contention on a single [[cats.effect.concurrent.Ref]] when
    ...
    *     [[cats.effect.concurrent.Ref]] and [[cats.effect.concurrent.Deferred]]
    *     to be created.
  ...
  /** Creates a cache implementation, which is able remove the stale values.
    ...
    * adds [[cats.effect.Timer]], to have the ability to schedule cache clean up
    * in a concurrent way.
    *
    * Minimal usage example:
    * {{{
  ...
  /** Creates [[Cache]] interface to a set of precreated caches.
    ...
    * partitions, so there is no contention on a single
    * [[cats.effect.concurrent.Ref]] when cache need to be updated.
    *
  ...
  implicit class CacheOps[F[_], K, V](val self: Cache[F, K, V]) extends AnyVal {
    ...
    /** Gets a value for specific key, or loads it using the provided function.
      ...
      */
    def getOrUpdateResource(key: K)(value: => Resource[F, V])(implicit F: BracketThrow[F]): F[V] = {
      self
        .getOrUpdate1(key) {
          value
            .allocated
