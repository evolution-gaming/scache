...
/**
 * Frozen copy of `LoadingCache` as it was before the cache was rebuilt on `MapRef`: the whole map
 * lives in a single `Ref[F, Map[K, EntryRef]]`, so every insertion or removal of any key CASes it,
 * with `MaxRetries` as the safety net. Kept here only so the benchmarks can measure the old and the
 * new implementation in one run, do not change it.
 */
                                  ...
                                  deferred
                                    ...
                                    .flatMap {
                                      ...
                                      case true =>
                                        ...
                                        def releaseAndReturnValue(state: EntryState.Value[F, V])
                                        ...
                                        def releaseAndReturnLoading(state: EntryState.Loading[F, V])
                                          : F[Either[A, Either[F[V], V]]] =
                                          entry
                                            .release1
                                            .start
